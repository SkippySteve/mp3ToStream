...
while numNotOne != 1:	#just ensuring the loop never ends...
	...
	subprocess.run(["ffmpeg", "-loglevel", "error", "-y", "-re", "-loop", "1", "-f", "image2", "-i", sys.argv[2], "-i", str(sys.argv[1]+currentTrack), "-codec:a", "copy", "-shortest", "-f", "flv", str("rtmp://a.rtmp.youtube.com/live2/"+sys.argv[3])])

#for text on screen, arg for ffmpeg: -vf "drawtext=fontfile=/path/to/font.ttf:text='currentTrack':fontcolor=white:fontsize=24:box=1:boxcolor=black@0.5:boxborderw=5:x=(w-text_w)/2:y=(h-text_h)/2"
